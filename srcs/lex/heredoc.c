...
/*   Updated: 2025/02/25 14:23:14 by mmiilpal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_filename(t_data *data)
{
	char	*temp_file;
	int		fd;
	char	*temp_num;
	...
	temp_file = ft_strjoin("/tmp/minidata_heredoc_", temp_num);
	free(temp_num);
	if (!temp_file)
	{
		perror("ft_strjoin");
...
void	cleanup_heredoc(int fd, t_data *data)
{
	if (fd != -1)
		close(fd);
	close(STDIN_FILENO);
...
void	create_heredoc(char *delimiter, t_data *data, int quote_status)
{
	int		fd;
	char	*line;
	char	*tmp;
	...
	{	
		...
		if (line == NULL)
		{
			if (g_exit_status != 130)
				ft_heredoc_warning(delimiter);
			break ;
		}
		tmp = line;
		write_line_to_heredoc(fd, tmp, data, quote_status);
	}
	if (line)
		free(line);
...
int	handle_heredoc(t_token *tmp, t_data *data, int option)
{
	...
	if (option == 0)
		ret = case_heredoc_syntax(data->tokens, data);
	if (data->heredoc)
		free(data->heredoc);
